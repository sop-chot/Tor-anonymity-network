...
func (t *OnionRPC) ReceiveRelayMessage(input resource.RequestMessage, output *resource.ResponseMessage) error {
	...
	print.Encryption(print.ONION_NODE, "ReceiveRelayMessage", "Decrypting relay message (showing partial)", input.EncryptedData[:20], decryptedData[:20])
...
func sendRelayCellHelper(command cells.OnionCommand, payload []byte, parentCircuitID string) error {
	...
	if parentExists {
		...
		print.Encryption(print.ONION_NODE, "sendRelayCellHelper", "encryting data packet (partial)", relayCell[:20], dataToSend[:20])
